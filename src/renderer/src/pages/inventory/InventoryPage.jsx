      ...
      try {
        ...
        if (metal === 0)
          summaryPromises = selectedItem.map((item) =>
            window.api.stock.getItemSummary({ name: item, metal: 'Gold' })
          )
        else
          summaryPromises = selectedItem.map((item) =>
            window.api.stock.getItemSummary({ name: item, metal: 'Silver' })
          )
        ...
        <div className="overflow-x-auto rounded-lg border mt-20 border-zinc-700">
          <table className="w-[40vw] min-w-full border-collapse  text-xl">
            <thead className="bg-zinc-800 text-zinc-300 sticky top-0">
              <tr>
                {['Item Name', 'Count', 'Weight'].map((head) => (
                  <th
                    key={head}
                ...
                return (
                  ...
                  >
                    <td
                      ...
                      onClick={() =>
                        nav(`/inventory/${metal === 0 ? 'Gold' : 'Silver'}/${item.name}`)
                      }
                    >
                      {item.name}
                    </td>
