...
export default function PurchaseItemByName() {
  ...
  useEffect(() => {
    const getItem = async (metal, name) => {
      try {
        const item = await window.api.purchase.getItemByName({ name, metal })
        setItem(item)
      } catch (error) {
          ...
          <table className="min-w-full border-collapse text-2xl text-zinc-200">
            ...
            <tbody>
              {items.map((item, idx) => (
                ...
                >
                  <td className="px-3 py-2 border-b border-zinc-800">{item.Bill.date}</td>
